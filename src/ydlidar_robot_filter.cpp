...
//This node removes points from the YDLIDAR's LaserScan that are the robot
//The removal of the robot also comes from adjusting the angle viewed by the YDLIDAR in the launch file
...
{
   //initialize
   ros::init(argc, argv, "ydlidar_robot_filter");
   ros::NodeHandle nh;

   //subscribe to LaserScan messages pulished by YDLIDAR
   ros::Subscriber sub = nh.subscribe<sensor_msgs::LaserScan>("/scan", 1, filter);

   //publisher for filteres scans
   pub = nh.advertise<sensor_msgs::LaserScan>("ydlidar_filtered", 1);
